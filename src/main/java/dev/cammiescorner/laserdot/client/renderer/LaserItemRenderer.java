package dev.cammiescorner.laserdot.client.renderer;
...
public class LaserItemRenderer  implements BuiltinItemRendererRegistry.DynamicItemRenderer, IdentifiableResourceReloader {
	...
	public CompletableFuture<Void> reload(Synchronizer synchronizer, ResourceManager manager, Profiler prepareProfiler, Profiler applyProfiler, Executor prepareExecutor, Executor applyExecutor) {
		return synchronizer.whenPrepared(Unit.INSTANCE).thenRunAsync(() -> {
			...
			inventoryItemModel = client.getBakedModelManager().getModel(new ModelIdentifier(LaserDot.id(itemId.getPath() + "_gui"), "inventory"));
			worldItemModel = client.getBakedModelManager().getModel(new ModelIdentifier(LaserDot.id(itemId.getPath() + "_handheld"), "inventory"));
			applyProfiler.pop();
